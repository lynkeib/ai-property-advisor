...
class FinancialCalculator:
    ...
    def calculate_monthly_mortgage(
        loan_amount: float, annual_interest_rate: float, years: int = 30
    ) -> float:
        """
    ...
    def calculate_metrics(
        ...
        total_interest_30_years = round(
            (monthly_mortgage * total_months) - loan_amount, 2
        )
        total_cost_30_years_excluding_principal = round(
            total_interest_30_years
            + ((monthly_property_tax + hoa + insurance) * total_months),
            2,
        )
